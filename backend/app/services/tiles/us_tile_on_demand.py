...
Generates *real* (non-synthetic) US-wide overlay tiles on-demand from Google Earth Engine
for the requested metric/date/tile coordinate, and returns a rendered PNG.

This is used as a fallback when pre-generated tiles are not present on disk.
"""
...
_us_data_service: USDataService | None = None
...
async def _get_us_data_service() -> USDataService:
    global _us_data_service

    if _us_data_service is not None and getattr(_us_data_service, "_initialized", False):
        return _us_data_service

    async with _service_lock:
        if _us_data_service is None:
            _us_data_service = USDataService()
        if not getattr(_us_data_service, "_initialized", False):
...
def resolve_us_tile_request(
    ...
    """
    Resolve the effective granularity and cache bucket for a US tile request.
...
def _build_metric_image(ee, metric: str, resolved: ResolvedTileRequest, geom) -> object:
    ...
    if metric == "temperature":
        ...
        image = ee.Image(ee.Algorithms.If(collection.size().gt(0), collection.mean(), ee.Image(0)))
        return image.subtract(273.15).clip(geom)
    ...
    if metric == "precipitation":
        ...
        return image.multiply(1000).clip(geom)
    ...
    if metric == "cropland":
        ...
        image = ee.Image(ee.Algorithms.If(collection.size().gt(0), collection.first(), ee.Image(0)))
        return image.clip(geom)
...
async def generate_us_tile_png(
    ...
    """
    Generate a single US tile PNG from real data sources (Earth Engine).

    Returns a transparent tile when the requested tile does not intersect the US bounds.
    """
    tile_west, tile_south, tile_east, tile_north = tile_bounds_mercator(z, x, y)

    # Quick reject for tiles outside our US bounding box.
    if (
        tile_east < US_BOUNDS_MERCATOR["west"]
        or tile_west > US_BOUNDS_MERCATOR["east"]
        or tile_north < US_BOUNDS_MERCATOR["south"]
        or tile_south > US_BOUNDS_MERCATOR["north"]
    ):
        return create_empty_tile()

    data_service = await _get_us_data_service()
    ee = data_service._ee  # Initialized in _get_us_data_service()
